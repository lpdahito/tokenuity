// [base]  CHAIN_ID=8453 npm run container
// [bsc]   CHAIN_ID=56 npm run container
...
export default async (): Promise<void> => {
  ...
  switch (Number(chainId)) {
    case 8453:
      ...
      command2 = `docker exec mongo-server mongorestore --noIndexRestore --uri mongodb://localhost:27017/trendsbase dump/base`
  ...
  let command3 = `docker run -d --restart on-failure --name explorer-c --link mongo-server --env-file .env --env CHAIN_ID=${chainId} -v /Users/lpdahito/projects/tokenuity/trends/trends-explorer/dist:/app/dist explorer`
  ...
  const { stdout: stdout3, stderr: stderr3 } = await execPromise(command3)
  console.log(stdout3); console.error(stderr3);
}